...
function [A_est, S_est] = HyperCSI_modified(X,N)
...
%------------------------ Step 5 & Step 6 ------------------------
comm_flag = 1;
eta = 1; % 0.9 is empirically good choice for endmembers in USGS library
for i = 1:N
    bbbb = b_hat;
    ccconst = h_hat;
    bbbb(:,i) = [];
...
if comm_flag == 1
    VV = C*alpha_hat;
    UU = d*ones(1,N)+(1e-8);
    closed_form_optval = max( 1 , max(max( (-VV) ./ UU)) ); % c' in Step 5  %move to the first quardant
